...
<div class="ui-page-narrow">
    <div class="ui-card">
        ...
        <form action="{{ route('candidates.update', $candidate) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name', $candidate->name) }}" required class="ui-input" />
                @error('name') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium mb-1" for="party">Party</label>
                <input id="party" type="text" name="party" value="{{ old('party', $candidate->party) }}" class="ui-input" />
                @error('party') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $candidate->is_active)) class="rounded border-slate-300" />
